...
namespace Sample.Elasticsearch.Domain.Application
{
    public class ActorsApplication : IActorsApplication
    {
        ...
        public ICollection<IndexActors> GetActorsAllCondition(string term)
        {
            ...
            query = query && new QueryContainerDescriptor<IndexActors>().Wildcard(w => w.Field(f => f.Name).Value($"*{term}*"))
                    || new QueryContainerDescriptor<IndexActors>().Wildcard(w => w.Field(f => f.Description).Value($"*{term}*"))
